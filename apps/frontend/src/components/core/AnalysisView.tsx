...
import { useParams, useRouter } from "next/navigation"; // [NEW] Import Navigasi
import { useCv, CriticalGap } from "@/lib/cv-context"; 
import { ArrowRight, Sparkles, AlertTriangle, ChevronLeft, Loader2, Info, X, ChevronRight, Zap, Check, ChevronDown, Lightbulb } from "lucide-react";
...
export function AnalysisView() {
  ...
  const handleCustomize = async (mode: 'analysis' | 'job_desc') => {
    ...
    try {
        // [NEW] Panggil Backend NestJS (Memicu Queue)
        // Kita tidak perlu kirim File atau Context JSON lagi, Backend sudah punya di DB.
        ...
        if (!res.ok) throw new Error("Failed to start customization");

        // [NEW] Redirect ke Halaman Editor (Polling akan terjadi di sana)
        router.push(`/cv/${cvId}/editor`);

    } catch (e) {
        console.error(e);
        alert("Gagal memproses permintaan AI. Coba lagi.");
  ...
  return (
    <div className="max-w-6xl mx-auto w-full relative pb-20">
       ...
       <motion.button 
          ...
          onClick={() => router.push('/')} // [NEW] Redirect ke Home
       ...
       <motion.div 
          ...
          initial="hidden"
          animate="visible"
          className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-[32px] p-8 md:p-12 shadow-2xl overflow-hidden relative"
       >
          ...
          <div className="grid lg:grid-cols-3 gap-8 mb-10">
              ...
              {/* RIGHT: CRITICAL GAPS & ACTION PLAN (AI GENERATED) */}
              <motion.div 
                variants={itemVariants} 
                className="bg-slate-950/30 border border-white/5 rounded-2xl p-1 flex flex-col h-full hover:border-red-500/20 transition-colors group"
              >
                 <div className="p-5 pb-2">
                    ...
                    <p className="text-xs text-slate-500 mb-4">AI-Recommended actions to improve your profile.</p>
                 </div>
          ...
          <motion.div 
            variants={itemVariants}
            className="bg-gradient-to-r from-indigo-900/40 to-slate-900 rounded-2xl p-1 border border-indigo-500/20"
          >
...
// ... Copy paste sub component (GapActionCard, ScoreBox, helpers) dari kode Anda sebelumnya ...
// ... Pastikan GapActionCard, ScoreBox, getScoreTextColor, dll tetap ada di bawah sini ...

// Kartu Accordion untuk menampilkan Gap + Action
...
function getBorderColor(score: number) {
    if (score >= 80) return "group-hover:border-emerald-500/30";
    if (score >= 60) return "group-hover:border-amber-500/30";
    return "group-hover:border-red-500/30";
}
...
function ScoreBox({label, score, detail, onClick}: {label: string, score: number, detail: string, onClick: () => void}) {
   ...
   return (
      <div 
        onClick={onClick}
        className={`bg-slate-950/40 p-5 rounded-2xl border border-white/5 cursor-pointer ${borderColor} hover:bg-slate-900/60 transition-all group flex flex-col justify-between h-full relative overflow-hidden`}
      >
         ...
         <div className="relative z-10">
             <div className="flex justify-between items-center mb-3">
                ...
                <div className={`text-xs font-bold px-2 py-0.5 rounded-full bg-white/5 ${textColor}`}>
                    {score}/100
                </div>
             </div>
             ...
             <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 h-9">
                {detail || "No analysis details available."}
             </p>
         </div>
         ...
         <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between relative z-10">
            ...
            <div className="flex items-center gap-1 text-[11px] text-indigo-400 font-bold opacity-70 group-hover:opacity-100 group-hover:translate-x-1 transition-all">
